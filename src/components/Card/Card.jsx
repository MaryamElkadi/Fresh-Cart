...
export default function Card({ productInfo }) {
    ...
    // Initialize state from localStorage or default to green
    const [isFavorite, setIsFavorite] = useState(() => {
        const savedFavorites = JSON.parse(localStorage.getItem('favorites')) || {}
        return savedFavorites[_id] || false
    })

    // Update localStorage when favorite status changes
    useEffect(() => {
        const savedFavorites = JSON.parse(localStorage.getItem('favorites')) || {}
        savedFavorites[_id] = isFavorite
    ...
    return (
        <div className="col-span-12 sm:col-span-6 md:col-span-4 lg:col-span-3 xl:col-span-2 shadow-lg rounded-md overflow-hidden">
            <div className='relative'>
                ...
                <div className="opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2 layer absolute w-full h-full left-0 top-0 bg-black bg-opacity-15">
                    <div 
                    ...
                    >
                        <i 
                            className={`fa-solid fa-heart ${isFavorite ? 'text-red-500' : 'text-green-500'}`}
                        ></i>
                    </div>
                    <div 
