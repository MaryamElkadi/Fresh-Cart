...
export default function Navbar() {
    ...
    useEffect(() => {
        ...
        const favoriteProducts = Object.keys(savedFavorites)
            .filter(key => savedFavorites[key])
            .map(key => key);
                    ...
                    {token && (
                        <ul className="flex items-center gap-6">
                            <li>
                                <NavLink
                                    className={({ isActive }) => {
                                        return `relative hover:before:w-full before:transition-[width] before:duration-300 before:h-[2px] before:absolute before:bg-primary before:left-0 before:-bottom-1 ${isActive ? "font-bold before:w-full" : "before:w-0"
                                            }`;
                                    }}
                                    to="/"
                                >
